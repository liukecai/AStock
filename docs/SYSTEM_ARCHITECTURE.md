...
```math
BurstZ =
\frac{x - mean(x_{previous\ 5\ days})}
{\max(std(x_{previous\ 5\ days}), 1)}
```

当前按自然日计算前五天，并使用最小标准差 `1` 防止零方差导致数值爆炸。
