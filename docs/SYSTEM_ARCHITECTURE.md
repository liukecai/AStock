...
> **核心前提**：系统已积累足够历史行情与信号数据，但评分权重（0.5 趋势 + 0.3 情绪 + 0.2 量能）尚未经回测验证。在引入实盘逻辑之前，必须先建立量化验证闭环。

- [x] 1. 引入中文金融情绪模型和 FinBERT（已于 V1.4 完成：`yiyanghkust/finbert-tone-chinese`，部署于本地高性能 PC）；
- [ ] 2. **[最高优先级] 构建历史回测框架**：基于已有 `signals` + `daily_prices` Parquet，计算因子 IC、五分位分层收益、Sharpe 和最大回撤，并纳入涨跌停约束（无法成交信号排除）；
- [ ] 3. **[高优先级] 保存 NLP 模型版本与推理证据**：在 `news_items` 表增加 `model_version`、`score_source`（`model` / `rule`）、`model_raw_output` 字段，支持回测期间情绪因子可审计；
- [ ] 4. 根据回测 IC 结果重新校准因子权重及研究权重阈值；
