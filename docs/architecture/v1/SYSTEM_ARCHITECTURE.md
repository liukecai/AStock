...
| `url` | 原文链接 |
| `sentiment` | `[-1, 1]` 情绪分 |
| `model_version` | 模型 ID 或规则版本，如 `yiyanghkust/finbert-tone-chinese` |
| `score_source` | `model` 或 `rule` |
| `model_raw_output` | 模型概率/标签或规则命中结果，供回测审计 |
...
这是无有效校准文件时的保守默认权重。`calibrate` 命令会按各基础因子的正
平均 IC、IC 胜率和样本期收缩系数重新归一化权重；少于 20 个有效 IC 截面时拒绝
生成校准文件。每条信号的 `metrics` 保存实际使用的权重、阈值和校准来源。
...
系统根据综合分生成 0–8% 的研究权重上限。下表为无校准文件时的默认阈值；
校准后使用历史可执行信号综合分的 40%、60%、80%、90% 分位：
...
7. 当前仓库样例仅包含单日信号，尚不足以生成可靠的生产校准参数；
8. `model-service` 宕机期间入库的新闻使用可审计的规则降级分，恢复后仍需手动重评。
...
> **核心前提**：回测与校准链路已经建立，但生产权重只有在积累至少 20 个有效
> IC 截面和 100 条可执行交易后才允许生成。在达到门槛前继续使用默认权重。
...
> 以下目标均依赖回测结论。若回测显示综合评分无显著预测力，应优先修正因子，而非推进实盘。
