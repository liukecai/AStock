...
| ORM 模型 | SQLAlchemy 模型，兼容 SQLite 和 PostgreSQL |
...
2. 能创建关系并查询邻居。
3. 能绑定证据到关系。
4. SQLAlchemy 模型不使用 SQLite 专有语法。
5. 不影响 V1 现有功能。
...
| 类型 | 内容 |
|---|---|
| 迁移脚本 | `yaml_to_kg_loader.py`：解析 YAML → 归一化实体 → Upsert Entity → Upsert Relation → 标记 `source_type = yaml_seed` |
| 关系增强 | 关系置信度字段和来源字段填充 |
| 验证 | 迁移前后规则覆盖度对比 |
