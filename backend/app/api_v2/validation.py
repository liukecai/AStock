...
def get_validation_event(event_id: str):
    results = db.rows("SELECT * FROM validation_results WHERE event_id=?", (event_id,))
...
def get_validation_event_stock(event_id: str, stock_code: str):
    result = db.row("SELECT * FROM validation_results WHERE event_id=? AND symbol=?", (event_id, stock_code))
    if not result:
...
def get_validation_summary(summary_type: str = Query(...), key: Optional[str] = None):
    ...
    summaries = db.rows(f"SELECT * FROM validation_summaries WHERE {where}", tuple(params))
